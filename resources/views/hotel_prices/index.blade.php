...
@section('content')
    ...
    <div id="loading" class="loading"><div><img src="/svg/spinner.svg"><br><span>Please Wait...</span></div></div>
        ...
        $(function() {
            ...
            var oTable = $('#hotel_prices').DataTable({
                ...
                @if(!isset($id))
                    deferLoading: false,
                @endif
                dom: "<'row'<'col-sm-2'li><'col-sm-2'B><'col-sm-3'<'jump_to_page'>><'col-sm-5'<'#statistics.text-right'>p>>rt<'bottom'ip><'clear'>",
                buttons: [{
                          text: 'Export CSV',
                ...
                ajax: {
                    ...
                    dataFilter: function(response) {
                        ...
                        $("#statistics").html('<div><span class="p_badge"><b>Max: </b>&euro;'+statistics['max_price'].toFixed(2)+'</span>|<span class="p_badge"><b>Min: </b>&euro;'+statistics['min_price'].toFixed(2)+'</span>|<span class="p_badge"><b>Average: </b>&euro;'+statistics['avg_price'].toFixed(2)+'</span></div>');
                        return response;
                    },
                },
                columns: [
            ...
            var form_html = '<form class="form-inline"><div class="form-group"><label>Page:</label><input type="number" class="form-control" id="page_number" style="width:90px"></div><button type="button" class="btn btn-default" id="go_to_page">Go</button></form>';

            $('.jump_to_page').html(form_html);

            $('#go_to_page').on('click', function(e) {
                var page_nubmer = parseInt($('#page_number').val()) - 1;
                oTable.page( page_nubmer ).draw('page');
            });            
